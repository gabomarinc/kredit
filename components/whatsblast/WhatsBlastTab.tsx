...
export const WhatsBlastTab: React.FC<WhatsBlastTabProps> = ({ prospects: sourceProspects }) => {
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            ...
            if (success) {
                addNotification(`✅ Campaña "${campaignToDelete.name}" eliminada exitosamente`);
    ...
    return (
        ...
        {showDeleteConfirm && campaignToDelete && createPortal(
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                <div
                    className="absolute inset-0 bg-black/20 backdrop-blur-sm transition-opacity"
                    onClick={handleDeleteCancel}
                ></div>
                <div className="bg-white border-2 border-red-200 rounded-[2rem] p-6 sm:p-8 max-w-md w-full shadow-2xl relative z-10 animate-fade-in-up">
