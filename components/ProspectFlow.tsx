...
import { PropertyType, UserPreferences, FinancialData, PersonalData, CalculationResult, Company } from '../types';
    ...
    <div className="flex gap-3 mb-8 justify-center">
      {Array.from({ length: totalSteps }).map((_, idx) => {
        ...
        return (
          <div
            key={idx}
            className={`h-1.5 rounded-full transition-all duration-500 ${isActive ? 'bg-indigo-500' : 'bg-gray-200'
              } ${isCurrent ? 'w-8' : 'w-4'}`}
          />
        );
      })}
...
export const ProspectFlow: React.FC<ProspectFlowProps> = ({ availableZones, companyName = "Krêdit", isEmbed = false, formId = null }) => {
  ...
  // Fetch company data and Form Config if embed
  useEffect(() => {
    const fetchCompanyData = async () => {
      // Priorizar formId, luego companyId del URL, luego localStorage (si no es embed)
      const params = new URLSearchParams(window.location.search);
      const urlCompanyId = params.get('company_id');
      const targetCompanyId = isEmbed ? (urlCompanyId || company?.id) : (localStorage.getItem('companyId') || company?.id);

      if (targetCompanyId) {
        try {
          const companyData = await getCompanyById(targetCompanyId.toString());
          if (companyData) {
            setCompany(companyData);
            setCompanyLogo(companyData.logoUrl || null); // Sync existing logo state

            // Set plan and role if they exist on companyData
            if (companyData.plan) setCompanyPlan(companyData.plan);
            if (companyData.role) setCompanyRole(companyData.role);

            setZones(companyData.zones.length > 0 ? companyData.zones : availableZones);
            // Default Requested Documents
            if (companyData.requestedDocuments) {
              setRequestedDocuments(companyData.requestedDocuments);
            }
          }

          // -- FORM CONFIG OVERRIDE --
          if (formId) {
            console.log('🔄 Cargando configuración del formulario:', formId);
            const form = await getFormById(formId);
            if (form && form.config) {
              console.log('✅ Configuración de formulario cargada:', form.config);

              // Override Zones
              if (form.config.zones && form.config.zones.length > 0) {
                setZones(form.config.zones);
              }

              // Override Requested Documents
              if (form.config.requestedDocuments) {
                setRequestedDocuments(form.config.requestedDocuments);
              }

              // Override APC Document (if we have a field for it in state/company)
              if (form.config.apcDocumentId) {
                setCompany(prev => prev ? ({ ...prev, apcDocumentDriveId: form.config!.apcDocumentId }) : null);
              }
            }
          }
    ...
    fetchCompanyData();
  }, [company?.id, isEmbed, availableZones, formId]);
          ...
          <div className="animate-fade-in-up">
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-10">
              <button
                ...
                className={`group text-left p-6 md:p-8 rounded-[2rem] border-2 transition-all duration-300 relative overflow-hidden ${preferences.propertyType === PropertyType.Apartment
                  ? 'border-indigo-100 bg-indigo-50/30 shadow-lg scale-[1.02]'
                  : 'border-gray-100 bg-white hover:border-indigo-50 hover:shadow-md'
                  }`}
              >
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-colors ${preferences.propertyType === PropertyType.Apartment ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-400 group-hover:bg-indigo-50 group-hover:text-indigo-500'
                  }`}>
              ...
              <button
                ...
                className={`group text-left p-6 md:p-8 rounded-[2rem] border-2 transition-all duration-300 relative overflow-hidden ${preferences.propertyType === PropertyType.House
                  ? 'border-indigo-100 bg-indigo-50/30 shadow-lg scale-[1.02]'
                  : 'border-gray-100 bg-white hover:border-indigo-50 hover:shadow-md'
                  }`}
              >
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-colors ${preferences.propertyType === PropertyType.House ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-400 group-hover:bg-indigo-50 group-hover:text-indigo-500'
                  }`}>
                  ...
                  <div className="flex items-center gap-3">
                    {[1, 2, 3, 4].map(num => (
                      <button
                        ...
                        className={`flex-1 h-14 rounded-2xl flex items-center justify-center transition-all font-bold text-lg ${preferences.bedrooms === num
                  ...
                  <div className="flex items-center gap-3">
                    {[1, 2, 3, 4].map(num => (
                      <button
                        ...
                        className={`flex-1 h-14 rounded-2xl flex items-center justify-center transition-all font-bold text-lg ${preferences.bathrooms === num
            ...
            <div className="mt-12 flex justify-center">
              <button
                ...
                className={`flex items-center gap-2 px-10 py-4 rounded-full font-semibold transition-all duration-500 ${preferences.zone.length > 0
          ...
          <div className="animate-fade-in-up max-w-2xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 px-4">
              ...
              <button
                ...
                className={`flex items-center justify-center gap-2 px-6 sm:px-8 py-4 sm:py-3 rounded-full font-semibold text-base sm:text-base transition-all duration-300 order-1 sm:order-2 flex-1 sm:flex-initial ${financial.familyIncome >= 800 && financial.familyIncome > 0
          ...
          <div className="animate-fade-in-up max-w-2xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 px-4">
              ...
              <button
                ...
                className={`flex items-center justify-center gap-2 px-6 sm:px-8 py-4 sm:py-3 rounded-full font-semibold text-base transition-all duration-300 order-1 sm:order-2 flex-1 sm:flex-initial ${!personal.fullName || !personal.email || !personal.phone || isSaving
          ...
          <div className="animate-fade-in-up">
            ...
            <div className="mt-10 flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 px-4 max-w-4xl mx-auto">
              ...
              <button
                ...
                className={`flex items-center justify-center gap-3 px-6 sm:px-10 py-4 rounded-full font-bold text-base sm:text-lg transition-all duration-300 shadow-xl order-1 sm:order-2 flex-1 sm:flex-initial ${(requestedDocuments.idFile && !personal.idFile) ||
                  (requestedDocuments.fichaFile && !personal.fichaFile) ||
                  (requestedDocuments.talonarioFile && !personal.talonarioFile) ||
                  (requestedDocuments.signedAcpFile && !personal.signedAcpFile) ||
                  isSaving
          ...
          <div className="bg-white rounded-[2rem] w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl relative animate-fade-in-up">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <span className={`px-4 py-1.5 rounded-lg text-sm font-bold text-white ${selectedProperty.type === 'Venta' ? 'bg-purple-600' : 'bg-green-600'
                  }`}>
